...
from warehouse_ddd.domain import exceptions
from warehouse_ddd.domain import model


def test_prefers_warehouse_batches_to_shipments():
    """
    arrange: создать батч, доступный на складе, и батч
        находящийся в пути; создать товарную позицию;
    act: запросить сервис allocate;
    assert: убедиться, что размещение произошло в складском
    батче (уменьшилось доступное количество),
...
def test_raises_out_of_stock_exception_if_cannot_allocate():
    ...
    assert: убедиться, что сервис allocate выбросил
        ошибку OutOfStock
    """
